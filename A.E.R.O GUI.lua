...
--- 7. ABA DIVERSÃO (TRANSPARENTE, CAMUFLAGEM DINÂMICA E SEIZURE) ---
...
local function salvarAparenciaOriginal()
	if player.Character then
		for _, part in ipairs(player.Character:GetDescendants()) do
...
camoBtn.MouseButton1Click:Connect(function()
	...
	if camuflagemAtiva then
		salvarAparenciaOriginal()
	else
		-- Restaura a aparência original do personagem ao desligar
		if player.Character then
...
-- Raycast continuo para mudar a cor para o bloco/parede que o jogador estiver encostando
RunService.RenderStepped:Connect(function()
	if camuflagemAtiva and player.Character and player.Character:FindFirstChild("HumanoidRootPart") then
		local hrp = player.Character.HumanoidRootPart
		
		-- Lança feixes de detecção (Raycast) para baixo e para frente
		local rayParams = RaycastParams.new()
		rayParams.FilterAncestorsOfInstances = {player.Character}
		rayParams.FilterType = Enum.RaycastFilterType.Exclude
		
		local rayDown = workspace:Raycast(hrp.Position, Vector3.new(0, -5, 0), rayParams)
		local rayFront = workspace:Raycast(hrp.Position, hrp.CFrame.LookVector * 4, rayParams)
		
		local hit = rayFront or rayDown
		if hit and hit.Instance then
			local wallColor = hit.Instance.Color
			local wallMaterial = hit.Instance.Material
